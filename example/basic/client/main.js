...
var Main = function() {
	...
	this.clientSocket.events.on("Direct Message",(function($this) {
		var $r;
		var e = haxe_ds_Either.Left(function(data,sender) {
			console.log("Server sent a 'Direct message' with data " + data);
		});
		$r = e;
		return $r;
	}(this)));
	this.clientSocket.events.on("Server wide broadcast",(function($this) {
		var $r;
		var e1 = haxe_ds_Either.Right(function(data1) {
...
var mphx_client_WebsocketClient = function(_ip,_port) {
	this.ready = false;
	this.events = new mphx_core_EventManager();
	this.ip = _ip;
	this.port = _port;
	this.messageQueue = [];
...
var mphx_core_EventManager = function() {
	this.eventMap = new haxe_ds_StringMap();
};
mphx_core_EventManager.__name__ = true;
mphx_core_EventManager.prototype = {
	on: function(eventName,event) {
		this.eventMap.set(eventName,event);
	}
	,callEvent: function(eventName,data,sender) {
		if(this.eventMap.exists(eventName) == true) {
			var _g;
			var this1 = this.eventMap.get(eventName);
			_g = this1;
			switch(_g[1]) {
			case 0:
				var eventWithSender = _g[2];
				eventWithSender(data,sender);
				break;
			case 1:
				var eventWithoutSender = _g[2];
				eventWithoutSender(data);
				break;
			}
		}
	}
	,__class__: mphx_core_EventManager
};
var mphx_core__$EventManager_Either_$Impl_$ = {};
mphx_core__$EventManager_Either_$Impl_$.__name__ = true;
mphx_core__$EventManager_Either_$Impl_$._new = function(e) {
	return e;
};
mphx_core__$EventManager_Either_$Impl_$.get_type = function(this1) {
	return this1;
};
mphx_core__$EventManager_Either_$Impl_$.fromLeft = function(left) {
	var e = haxe_ds_Either.Left(left);
	return e;
};
mphx_core__$EventManager_Either_$Impl_$.fromRight = function(right) {
	var e = haxe_ds_Either.Right(right);
	return e;
};
var mphx_server_Room = function() {
	this.maxConnections = 0;
	this.full = false;
	this.connections = [];
};
mphx_server_Room.__name__ = true;
mphx_server_Room.prototype = {
	onLeave: function(client) {
		HxOverrides.remove(this.connections,client);
	}
	...
	,broadcast: function(event,data) {
		...
		while(_g < _g1.length) {
			var client = _g1[_g];
			++_g;
			if(client.send(event,data)) success = false;
		}
		return success;
	}
...
mphx_tcp_Connection.prototype = {
	...
	,onAccept: function(cnx) {
		this.cnx = cnx;
	}
	,putInRoom: function(newRoom) {
		if(newRoom.full) return false;
		if(this.room != null) this.room.onLeave(this);
		this.room = newRoom;
		newRoom.onJoin(this);
		return true;
	}
	,loseConnection: function(reason) {
		console.log("Client disconnected with code: " + reason);
		if(this.cnx != null) {
			this.cnx.close();
			this.cnx = null;
		}
	...
	,isConnected: function() {
		return this.cnx != null && this.cnx.isOpen();
	}
	,send: function(event,data) {
		var object = { t : event, data : data};
		var serialiseObject = JSON.stringify(object);
		var result = this.cnx.writeBytes(haxe_io_Bytes.ofString(serialiseObject + "\r\n"));
		return result;
	}
	,recieve: function(line) {
		console.log(line);
		var msg = JSON.parse(line);
		this.events.callEvent(msg.t,msg.data,this);
	}
	,dataReceived: function(input) {
		var line = "";
		try {
			line = input.readLine();
		} catch( e ) {
	...
	,__class__: mphx_tcp_Connection
};
var mphx_tcp_NetSock = function(socket) {
};
mphx_tcp_NetSock.__name__ = true;
mphx_tcp_NetSock.prototype = {
	isOpen: function() {
		return true;
	}
